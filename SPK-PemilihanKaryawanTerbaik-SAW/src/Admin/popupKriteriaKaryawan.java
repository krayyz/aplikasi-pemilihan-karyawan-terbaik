...
@author JR
*/
public class popupKriteriaKaryawan extends javax.swing.JFrame {
    private static Connection conn = new koneksi().configDB();
    private DefaultTableModel tabmode, tabmodeL;
    public admin Kriteria = null;
    
    public popupKriteriaKaryawan() {
        initComponents();
        DataKaryawan();
    }

    public void DataKaryawan(){
        Object [] Baris ={"ID Presensi", "NIK", "Nama"};
        tabmode = new DefaultTableModel(null, Baris);
        String Cari = txtSearch.getText();
        try{
            String SQL = "SELECT presensi.Id_presensi, karyawan.NIK, karyawan.Nama"
                    + " FROM presensi INNER JOIN karyawan ON presensi.NIK = karyawan.NIK";
    ...
    private void tbKaryawanMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbKaryawanMouseClicked
        ...
        Kriteria.idpresensi = tbKaryawan.getValueAt(row, 0).toString();
        Kriteria.nik = tbKaryawan.getValueAt(row, 1).toString();
        try{
            String sql = "SELECT COUNT(Kehadiran) from presensi where NIK='"+Kriteria.nik+"' AND Kehadiran='Hadir'";
            Statement St = conn.createStatement();
            ResultSet Res = St.executeQuery(sql);
            while (Res.next()) {
                Kriteria.hadir = Res.getInt(1);
            }
        }catch(SQLException exc){
            JOptionPane.showMessageDialog(this, "data gagal dipanggil"+exc);
        }
        Kriteria.DataKriteriaTerpilih();
        this.dispose();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(popupKriteriaKaryawan.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new popupKriteriaKaryawan().setVisible(true);
            }
        });
    }
